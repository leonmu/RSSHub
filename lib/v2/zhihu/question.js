...
module.exports = async (ctx) => {
    ...
    const cookie_mes = await ctx.cache.tryGet('zhihu:cookies:d_c0', async () => {
        ...
        const question_data = JSON.parse($('script[id="js-initialData"]').eq(0).text());
        const questions = question_data.initialState.entities.questions;
        ...
        question_description =
            JSON.stringify({ pv: question.visitCount, answerCount: question.answerCount, followerCount: question.followerCount, voteupCount: question.voteupCount, created: question.created }) +
                '-$-' +
                question.detail.replace(/<[^>]*>/g, '<br/>').replace(/(?:<br\/>)+/g, '') || '';
